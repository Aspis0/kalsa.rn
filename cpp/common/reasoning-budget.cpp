...
static void common_reasoning_budget_accept(struct llama_sampler * smpl, llama_token token) {
    ...
    switch (ctx->state) {
        ...
        case REASONING_BUDGET_FORCING:
            // force_pos is advanced in apply(), not here.  This keeps the
            // first forced token available when the sampler starts directly
            // in FORCING state (for example immediate activation with budget 0).
            break;
...
static void common_reasoning_budget_apply(struct llama_sampler * smpl, llama_token_data_array * cur_p) {
    ...
    // Advance after applying the token, so the next generation step sees the
    // next forced token rather than consuming the first one in accept().
    ctx->force_pos++;
    if (ctx->force_pos >= ctx->forced_tokens.size()) {
        ctx->state = REASONING_BUDGET_DONE;
