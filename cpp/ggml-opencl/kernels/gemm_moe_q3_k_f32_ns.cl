...
// ORACLE=GEMV unpack — same extracts as kernel_dequant_row_q3_k_trans4_ns
static inline half16 dequant16_q3(uint bits, uint sc, uint qh_w, uint lane, float d, float scale_zero, uint m03, uint m01) {
    float dl = d * (convert_float(sc) - scale_zero);
    half16 a;
    #define Q3LANE(k, dst) \
        { \
            uint q_u = (bits >> (2 * (uint)(k))) & m03; \
            uint hm  = (qh_w >> (lane + (uint)(k))) & m01; \
            int qhat = (int)q_u; \
            if (hm == 0) { qhat = (int)q_u - 4; } \
            dst = (half)(dl * convert_float(qhat)); \
        }
    Q3LANE(0,  a.s0) Q3LANE(1,  a.s1) Q3LANE(2,  a.s2) Q3LANE(3,  a.s3)
    Q3LANE(4,  a.s4) Q3LANE(5,  a.s5) Q3LANE(6,  a.s6) Q3LANE(7,  a.s7)
    Q3LANE(8,  a.s8) Q3LANE(9,  a.s9) Q3LANE(10, a.sa) Q3LANE(11, a.sb)
    Q3LANE(12, a.sc) Q3LANE(13, a.sd) Q3LANE(14, a.se) Q3LANE(15, a.sf)
    #undef Q3LANE
    return a;
}

__attribute__((qcom_wave_pair_mode(1)))
kernel void kernel_gemm_moe_q3_k_f32_ns(
    ...
    uint expert_qh_offset = (uint)expert_id * (ne00 / 32) * ne01;
    ...
    for (uint sb = 0; sb < num_superblocks; ++sb) {
        ...
        for (uint g = 0; g < 16; ++g) {
            ...
            uint hi   = g & 1;
            uint lane = hi << 4;
            uint qh_w = src0_qh[expert_qh_offset + (sb * 8 + p) * ne01 + row_load];
            reg_a = dequant16_q3(bits, sc, qh_w, lane, d, scale_zero, m03, m01);
