...
namespace rnllama_jsi {
    ...
    inline json tokenResultJson(rnllama::llama_rn_context* ctx, const rnllama::completion_token_output& token) {
        ...
        if (token.raw_probs_requested) {
            res["tok"] = (int) token.tok;
            res["forced"] = token.forced;
            json rawProbs = json::array();
            for (const auto& p : token.raw_probs) {
                rawProbs.push_back(json::array({(int) p.tok, (double) p.logprob}));
            }
            res["raw_probs"] = std::move(rawProbs);
        }
    ...
    inline CompletionResult completionResult(rnllama::llama_rn_context* ctx) {
        ...
        t.prompt_n = governor_prefill
            ...
            : (int32_t) perf.n_p_eval;
        t.prompt_ms = governor_prefill
            ? (double) governor_stats.prefill_us / 1000.0
            : perf.t_p_eval_ms;
        t.prompt_per_token_ms = t.prompt_n > 0 ? t.prompt_ms / t.prompt_n : 0.0;
        t.prompt_per_second = t.prompt_ms > 0 ? 1e3 / t.prompt_ms * t.prompt_n : 0.0;
