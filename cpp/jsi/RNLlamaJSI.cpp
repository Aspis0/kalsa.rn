...
static llama_governor_thermo_profile governorThermoFromJsi(
    ...
    return rnllama::parse_governor_thermo(rnllama_jsi::toJson(runtime, value));
}
...
namespace rnllama_jsi {
    ...
    static llama_governor_thermo_profile parseGovernorThermo(const json& params) {
        const auto thermo = params.find("governor_thermo");
        if (thermo == params.end()) {
            throw std::invalid_argument("governor_thermo is required in governor mode");
        }
        return rnllama::parse_governor_thermo(*thermo);
    }
    ...
    void installJSIBindings(
        ...
        auto setGovernorThermo = jsi::Function::createFromHostFunction(runtime,
            ...
            [callInvoker](jsi::Runtime& runtime, const jsi::Value&, const jsi::Value* arguments, size_t count) -> jsi::Value {
                ...
                const auto profile = governorThermoFromJsi(runtime, arguments[1]);
                return createPromiseTask(runtime, callInvoker,
        ...
        auto completion = jsi::Function::createFromHostFunction(runtime,
            ...
            [callInvoker](jsi::Runtime& runtime, const jsi::Value& thisValue, const jsi::Value* arguments, size_t count) -> jsi::Value {
                ...
                const bool governorThermoProvided =
                    ctx->hasGovernor() && params.contains("governor_thermo");
                if (governorThermoProvided) {
                    governorThermo = parseGovernorThermo(params);
                }
                ...
                return createPromiseTask(runtime, callInvoker, [runtimePtr = std::shared_ptr<jsi::Runtime>(&runtime, [](jsi::Runtime*){}), contextId, onToken, emitPartial, mediaPaths, chat_format, reasoning_format, generation_prompt, chat_parser, prefill_text, governorThermo, governorThermoProvided, callInvoker]() -> PromiseResultGenerator {
                    auto ctx = getContextOrThrow(contextId);
