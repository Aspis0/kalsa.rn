...
exports.RNLLAMA_MTMD_DEFAULT_MEDIA_MARKER = exports.LlamaSpeaker = exports.LlamaContext = exports.BuildInfo = void 0;
exports.addNativeLogListener = addNativeLogListener;
exports.getBackendDevicesInfo = getBackendDevicesInfo;
Object.defineProperty(exports, "getTTSVoice", {
  enumerable: true,
  get: function () {
    return _ttsVoices.lookupVoice;
  }
});
exports.initLlama = initLlama;
exports.installJsi = void 0;
Object.defineProperty(exports, "listTTSLanguages", {
  enumerable: true,
  get: function () {
    return _ttsVoices.listLanguages;
  }
});
Object.defineProperty(exports, "listTTSVoices", {
  enumerable: true,
  get: function () {
    return _ttsVoices.listVoices;
  }
});
...
var _ttsVoices = require("./tts-voices");
...
exports.LlamaSpeaker = LlamaSpeaker;
class LlamaContext {
  gpu = false;
  reasonNoGPU = '';
  /**
  ...
  parallel = {
    ...
    completion: async (params, onToken) => {
      ...
      if (!nativeParams.prompt && !params.embedding && !nativeParams.media_paths) throw new Error('Prompt is required');
  ...
  async setGovernorThermo(profile) {
    const {
      llamaSetGovernorThermo
    } = getJsi();
  ...
  async embedding(text, params) {
    ...
    const {
      embedding
    } = await llamaEmbedding(this.id, text, params || {});
    return {
      embedding: Array.from(embedding)
    };
  }
  ...
  async getFormattedAudioCompletion(options) {
    ...
    if (cap.requiresPhonemes && options.phonemizer) {
      inputText = await Promise.resolve(options.phonemizer(options.prompt, language));
    }
    const {
    ...
    if (options.speaker instanceof LlamaSpeaker) {
      return llamaGetFormattedAudioCompletion(this.id, null, inputText, options.speaker.id);
    }
    ...
    if (options.speaker && typeof options.speaker === 'object') {
      ...
      const name = typeof options.speaker === 'string' ? options.speaker : 'default';
      ...
      const voice = cap.promptKind === 'outetts_v1_0' ? null : lookupVoice(cap.family, name, language);
      if (!voice && typeof options.speaker === 'string') {
        ...
        throw new Error(`Unknown built-in voice '${name}' for ${cap.family || 'this model'} (${language})`);
      }
      payload = voice ? {
        ...voice
      } : null;
    ...
    if (payload && cap.requiresPhonemes && !payload.ref_phones && payload.ref_text && options.phonemizer) {
      payload.ref_phones = await Promise.resolve(options.phonemizer(payload.ref_text, language));
    }
  ...
  async decodeAudioTokens(tokens) {
    ...
    const pcm = await llamaDecodeAudioTokens(this.id, tokens instanceof Int32Array ? tokens : Int32Array.from(tokens));
  ...
  async createSpeaker(config) {
    ...
    const h = await llamaCreateSpeaker(this.id, config.refAudio, {
      ...
      ...(config.emotion !== undefined ? {
        emotion: config.emotion
      } : {})
    });
    return new LlamaSpeaker(this.id, h);
  }
  async decodeAudioEmbeddings(embeddings, embeddingDim) {
    const {
      llamaDecodeAudioEmbeddings
    } = getJsi();
    const pcm = await llamaDecodeAudioEmbeddings(this.id, embeddings instanceof Float32Array ? embeddings : Float32Array.from(embeddings), embeddingDim);
    return Array.from(pcm);
  }
...
const normalizeFileUri = path => path?.startsWith('file://') ? path.slice(7) : path;
const normalizeSpeculativeDraftPaths = speculative => {
  if (!speculative || typeof speculative !== 'object' || Array.isArray(speculative)) {
    return speculative;
  }
  const {
    draft
...
async function getBackendDevicesInfo() {
  ...
  try {
    ...
    console.warn('[RNLlama] Failed to read backend devices info, falling back to empty list', e);
    return [];
  }
}
async function initLlama({
  ...
  if (nativeRest.cache_type_k && !validCacheTypes.includes(nativeRest.cache_type_k)) {
    console.warn(`[RNLlama] initLlama: Invalid cache K type: ${nativeRest.cache_type_k}, falling back to f16`);
    delete nativeRest.cache_type_k;
  }
  if (nativeRest.cache_type_v && !validCacheTypes.includes(nativeRest.cache_type_v)) {
    console.warn(`[RNLlama] initLlama: Invalid cache V type: ${nativeRest.cache_type_v}, falling back to f16`);
