...
static bool ggml_cpu_type_is_k_quant(enum ggml_type type) {
    switch (type) {
        case GGML_TYPE_Q2_K:
        case GGML_TYPE_Q3_K:
        case GGML_TYPE_Q4_K:
...
static bool ggml_cpu_mul_mat_id_should_fuse(
    ...
    if (!ggml_cpu_get_moe_fused_gemv()) {
        return false;
    }
    ...
    if (ggml_cpu_repack_extra_is_tiled(src0->extra)) {
        return false;
    }
    if (ids->ne[1] != 1) {
    ...
    return ggml_cpu_type_is_k_quant(src0->type);
}
...
static void moe_fused_gemv_resolve(void) {
    ...
    const char * e = getenv("GGML_MOE_FUSED_GEMV");
...
static void moe_fused_ffn_resolve(void) {
    ...
    const char * e = getenv("GGML_MOE_FUSED_FFN");
    atomic_store(&g_moe_fused_ffn, moe_fused_env_disabled(e) ? 0 : 1);
}

void ggml_cpu_set_moe_fused_gemv(bool enabled) {
    atomic_store(&g_moe_fused_gemv, enabled ? 1 : 0);
}

bool ggml_cpu_get_moe_fused_gemv(void) {
    moe_fused_gemv_resolve();
    return atomic_load(&g_moe_fused_gemv) != 0;
}

void ggml_cpu_set_moe_fused_ffn(bool enabled) {
    atomic_store(&g_moe_fused_ffn, enabled ? 1 : 0);
}

bool ggml_cpu_get_moe_fused_ffn(void) {
    moe_fused_ffn_resolve();
    return atomic_load(&g_moe_fused_ffn) != 0;
}

// S3 diagnosis: GGML_MOE_REPACK_TRACE=1 prints which MUL_MAT[_ID] path ran (extra-buffer REPACK
// vs stock vec_dot). GGML_MOE_FUSED_FFN_PROFILE=1 times fused phases A-D every 28 launches.
static int moe_repack_trace_on(void) {
    ...
    const char * e = getenv("GGML_MOE_REPACK_TRACE");
...
static int moe_fused_profile_on(void) {
    ...
    const char * e = getenv("GGML_MOE_FUSED_FFN_PROFILE");
    const int on = (e && e[0] && e[0] != '0') ? 1 : 0;
    atomic_store(&v, on);
    return on;
}
...
static void moe_repack_trace_op(const struct ggml_tensor * tensor, bool via_repack) {
    ...
    const char * buft = (src0 && src0->buffer) ? ggml_backend_buffer_name(src0->buffer) : "null";
...
static void ggml_compute_forward_mul_mat_id_fused(
        const struct ggml_compute_params * params,
        struct ggml_tensor * dst,
...
static void ggml_compute_forward_mul_mat_id(
    ...
    if (ggml_cpu_mul_mat_id_should_fuse(params, src0, ids)) {
        ggml_compute_forward_mul_mat_id_fused(params, dst, atomic_current_chunk);
        return;
    }
                ...
                case GGML_OP_MUL_MAT_ID:
                    {
                        ...
                        cur += ids->ne[0] * ggml_row_size(GGML_TYPE_Q8_0, src0->ne[1]) + 256;
                        cur += ggml_row_size(GGML_TYPE_Q8_K, src0->ne[0]) + 256;   // q_input slack if src0 is down-shaped
...
// ggml_can_fuse cannot match this span: gate/up MUL_MAT_IDs are siblings (both
...
static bool moe_ffn_is_reduce_src(
    ...
    if (t->op == GGML_OP_VIEW && t->src[0] == mul) {
        return true;
    }
    return t->op == GGML_OP_ADD || t->op == GGML_OP_CONT;
}
...
static bool moe_ffn_span_has_external_use(
    ...
    for (int i = first; i < last; ++i) {
        ...
        for (int j = i + 1; j <= last; ++j) {
            const struct ggml_tensor * other = cgraph->nodes[j];
        ...
        if (span_uses != ggml_node_get_use_count(cgraph, i)) {
            return true;
        }
    }
    return false;
...
static void ggml_compute_forward_moe_swiglu_id_fused(
        const struct ggml_compute_params * params,
    ...
    if (prof && ith == 0) {
        ...
        if (atomic_fetch_add_explicit(&fused_trace, 1, memory_order_relaxed) < 4) {
            const char * buft = (gate_exps->buffer) ? ggml_backend_buffer_name(gate_exps->buffer) : "null";
    ...
    const enum ggml_type type_gate = gate_exps->type;
    const enum ggml_type type_up   = up_exps->type;
    const enum ggml_type type_down = down_exps->type;
    ...
    enum ggml_type    const vdt_gate     = type_traits_cpu[type_gate].vec_dot_type;
    enum ggml_type    const vdt_down     = type_traits_cpu[type_down].vec_dot_type;
    ggml_from_float_t const from_float_h = type_traits_cpu[vdt_gate].from_float;
    ggml_from_float_t const from_float_d = type_traits_cpu[vdt_down].from_float;

    const size_t q_input_sz  = ggml_row_size(vdt_gate, n_embd);
    const size_t q_hidden_sz = ggml_row_size(vdt_down, n_ff);
    ...
    GGML_ASSERT(params->wsize >= (size_t) ((char *) wdata_cur - (char *) params->wdata));
    GGML_ASSERT(vec_dot_gate && vec_dot_up && vec_dot_down);
    GGML_ASSERT(dst->nb[0] == sizeof(float));

    const bool hidden_already_q = (hidden->type == vdt_gate) && ggml_is_contiguous(hidden);
    const char * q_input = hidden_already_q ? (const char *) hidden->data : q_input_buf;

    const bool down_is_f32 = (vdt_down == GGML_TYPE_F32);
    ...
    if (!hidden_already_q) {
        ...
        GGML_ASSERT(from_float_h);
        const size_t bs = ggml_blck_size(vdt_gate);
        ...
        if (n_elem > 0) {
            from_float_h(
                ...
                (void *) (q_input_buf + blk0 * ggml_type_size(vdt_gate)),
                n_elem);
        }
    }
        ...
        while (current_chunk < nchunk0 * nchunk1) {
            ...
            for (int64_t iir0 = ir0_start; iir0 < ir0_end; iir0 += blck_0) {
                ...
                while (ir0 < blk_end) {
                    ...
                    ggml_vec_swiglu_f32((int) nrun, hidden_ff + slot * n_ff + local, gate_tmp, up_tmp);
                    ir0 = run_end;
                }
            }
    ...
    if (!down_is_f32) {
        GGML_ASSERT(from_float_d);
        const size_t bs = ggml_blck_size(vdt_down);
        ...
        for (int slot = 0; slot < k; ++slot) {
            ...
            if (n_elem > 0) {
                from_float_d(
                    ...
                    (void *) (q_hidden + (size_t) slot * q_hidden_sz + (size_t) blk0 * ggml_type_size(vdt_down)),
                    n_elem);
            }
        }
    }
...
static int ggml_cpu_try_fuse_moe_ffn(
        const struct ggml_cgraph * cgraph,
        const int node_n,
        const struct ggml_compute_params * params) {

    if (!ggml_cpu_get_moe_fused_ffn()) {
        return 0;
    }
    ...
    struct ggml_tensor * n0 = cgraph->nodes[node_n];
    struct ggml_tensor * n1 = cgraph->nodes[node_n + 1];
    struct ggml_tensor * n2 = cgraph->nodes[node_n + 2];
    struct ggml_tensor * n3 = cgraph->nodes[node_n + 3];
    struct ggml_tensor * n4 = cgraph->nodes[node_n + 4];

    if (n0->op != GGML_OP_MUL_MAT_ID ||
        n1->op != GGML_OP_MUL_MAT_ID ||
        n2->op != GGML_OP_GLU ||
        n3->op != GGML_OP_MUL_MAT_ID ||
        n4->op != GGML_OP_MUL) {
        return 0;
    }
    ...
    if ((n0->src[0] && ggml_cpu_repack_extra_is_tiled(n0->src[0]->extra)) ||
        (n1->src[0] && ggml_cpu_repack_extra_is_tiled(n1->src[0]->extra)) ||
        (n3->src[0] && ggml_cpu_repack_extra_is_tiled(n3->src[0]->extra))) {
        return 0;
    }

    if ((n0->flags & GGML_TENSOR_FLAG_COMPUTE) == 0 ||
        (n1->flags & GGML_TENSOR_FLAG_COMPUTE) == 0 ||
        (n2->flags & GGML_TENSOR_FLAG_COMPUTE) == 0 ||
        (n3->flags & GGML_TENSOR_FLAG_COMPUTE) == 0 ||
        (n4->flags & GGML_TENSOR_FLAG_COMPUTE) == 0) {
        return 0;
    }

    if (ggml_get_glu_op(n2) != GGML_GLU_OP_SWIGLU) {
        return 0;
    }
    if (ggml_get_op_params_i32(n2, 1) != 0) {
        return 0; // swapped
    }
    ...
    struct ggml_tensor * gate_mm = n2->src[0];
    struct ggml_tensor * up_mm   = n2->src[1];
    if (!((gate_mm == n0 && up_mm == n1) || (gate_mm == n1 && up_mm == n0))) {
        return 0;
    }

    struct ggml_tensor * down_mm = n3;
    if (down_mm->src[1] != n2) {
        return 0;
    }
    ...
    const struct ggml_tensor * hidden    = gate_mm->src[1];
    const struct ggml_tensor * ids       = gate_mm->src[2];
    const struct ggml_tensor * gate_exps = gate_mm->src[0];
    const struct ggml_tensor * up_exps   = up_mm->src[0];
    const struct ggml_tensor * down_exps = down_mm->src[0];

    if (!hidden || !ids || !gate_exps || !up_exps || !down_exps) {
        return 0;
    }

    struct ggml_tensor * mul = n4;
    struct ggml_tensor * weights = (mul->src[0] == down_mm) ? mul->src[1] : mul->src[0];
    if (mul->src[0] != down_mm && mul->src[1] != down_mm) {
        return 0;
    }
    if (!weights || weights->type != GGML_TYPE_F32) {
        return 0;
    }
    ...
    if (!ggml_cpu_type_is_k_quant(gate_exps->type) || !ggml_cpu_type_is_k_quant(up_exps->type)) {
        return 0;
    }
    ...
    const enum ggml_type vdt_gate = type_traits_cpu[gate_exps->type].vec_dot_type;
    const enum ggml_type vdt_down = type_traits_cpu[down_exps->type].vec_dot_type;
    if (hidden->type != vdt_gate) {
        if (hidden->type != GGML_TYPE_F32 || !type_traits_cpu[vdt_gate].from_float) {
            return 0;
        }
    } else if (!ggml_is_contiguous(hidden)) {
        return 0; // kernel from_float / already-q path assumes a packed row
    }
    if (vdt_down != GGML_TYPE_F32 && !type_traits_cpu[vdt_down].from_float) {
        return 0;
    }
    ...
    while (p < cgraph->n_nodes) {
        struct ggml_tensor * t = cgraph->nodes[p];
        if (t->op == GGML_OP_VIEW && t->src[0] == mul) {
    ...
    {
        size_t need = 64;
        need += ggml_row_size(vdt_gate, n_embd) + 64;
        need += (size_t) k * (size_t) n_ff * sizeof(float) + 64;
        need += (size_t) k * ggml_row_size(vdt_down, n_ff) + 64;
    ...
    struct ggml_tensor * dst = cgraph->nodes[last];
    ggml_compute_forward_moe_swiglu_id_fused(
...
static int ggml_cpu_try_fuse_ops(
    ...
    {
        const int n_moe = ggml_cpu_try_fuse_moe_ffn(cgraph, node_n, params);
        if (n_moe > 0) {
            return n_moe;
        }
    }
