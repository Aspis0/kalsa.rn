...
static size_t ggml_repack_q23k_8x4_slice_stride(size_t tile_bytes, int64_t ne0, int64_t ne1) {
    const int64_t n_tiles = (ne1 / 8) * (ne0 / QK_K);
    const size_t  payload = (size_t) n_tiles * tile_bytes;
    return (payload + 63) & ~size_t(63);
}

static size_t ggml_repack_q23k_8x4_nbytes(size_t tile_bytes, int64_t ne0, int64_t ne1, int64_t ne2) {
    if (ne0 <= 0 || ne1 <= 0 || ne2 <= 0) {
        return 0;
    }
    return ggml_repack_q23k_8x4_slice_stride(tile_bytes, ne0, ne1) * (size_t) ne2;
}
...
void ggml_gemv_q2_K_8x4_q8_K_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int nb = n / QK_K;
    ...
    for (int x = 0; x < nc / ncols_interleaved; x++) {
        ...
        for (int b = 0; b < nb; b++) {
            ...
            for (int j = 0; j < 8; j++) {
                const float d    = GGML_CPU_FP16_TO_FP32(b_ptr[b].d[j]);
                const float dmin = GGML_CPU_FP16_TO_FP32(b_ptr[b].dmin[j]);
                int32_t     sum_sd = 0;
                int32_t     sum_mb = 0;
...
static inline int ggml_repack_q8kx4_bsum_index(int row, int sb) {
    return ((sb / 4) * 16) + (row * 4) + (sb % 4);
}

static inline int8_t ggml_repack_q8kx4_qs(const block_q8_Kx4 * a, int row, int k) {
    const int group = k / 4;
    const int in4   = k % 4;
    return a->qs[group * 16 + row * 4 + in4];
}

void ggml_gemm_q2_K_8x4_q8_K_generic(int n, float * GGML_RESTRICT s, size_t bs, const void * GGML_RESTRICT vx, const void * GGML_RESTRICT vy, int nr, int nc) {
    const int nb = n / QK_K;
    constexpr int ncols_interleaved = 8;
                ...
                for (int m = 0; m < 4; m++) {
                    ...
                    for (int j = 0; j < 8; j++) {
                        ...
                        for (int sb = 0; sb < 16; sb++) {
                            ...
                            for (int lane = 0; lane < 16; lane++) {
                                const int q = (b_ptr[b].qs[sb][j][lane / 4] >> ((lane % 4) * 2)) & 3;
                                dot += q * ggml_repack_q8kx4_qs(a, m, sb * 16 + lane);
                            }
                            sum_sd += scale * dot;
                            sum_mb += minv * (int32_t) a->bsums[ggml_repack_q8kx4_bsum_index(m, sb)];
                        }
                ...
                for (int m = 0; m < 4; m++) {
                    ...
                    for (int j = 0; j < 8; j++) {
                        ...
                        for (int sb = 0; sb < 16; sb++) {
                            ...
                            sum_sc += s_sc * (dot - 4 * (int32_t) a->bsums[ggml_repack_q8kx4_bsum_index(m, sb)]);
                        }
...
static int repack_q2_K_to_q2_K_8x4(struct ggml_tensor * t, const void * GGML_RESTRICT data, size_t data_size) {
    ...
    GGML_ASSERT(data_size == src_slice * (size_t) n_slices);

    const size_t       stride  = ggml_repack_q23k_8x4_slice_stride(tile_bytes, t->ne[0], t->ne[1]);
...
static int repack_q3_K_to_q3_K_8x4(struct ggml_tensor * t, const void * GGML_RESTRICT data, size_t data_size) {
...
template <> int repack<block_q2_K, 4, 8>(struct ggml_tensor * t, const void * data, size_t data_size) {
...
template <> void gemv<block_q2_K, 4, 8, GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemv_q2_K_8x4_q8_K(n, s, bs, vx, vy, nr, nc);
}

template <> void gemv<block_q3_K, 4, 8, GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
...
template <> void gemm<block_q2_K, 4, 8, GGML_TYPE_Q8_K>(int n, float * s, size_t bs, const void * vx, const void * vy, int nr, int nc) {
    ggml_gemm_q2_K_8x4_q8_K(n, s, bs, vx, vy, nr, nc);
}
...
template <typename BLOC_TYPE, int64_t INTER_SIZE, int64_t NB_COLS, ggml_type PARAM_TYPE> class tensor_traits : public tensor_traits_base {
    ...
    bool compute_forward(struct ggml_compute_params * params, struct ggml_tensor * op) override {
        if constexpr (k_q23k_8x4) {
            ...
            // Shape-based, not ggml_n_dims: n_as==1 is [K,N,1] so n_dims==2.
            if (op->op == GGML_OP_MUL_MAT_ID && op->src[1] && op->src[2] &&
    ...
    void forward_mul_mat_q23k(ggml_compute_params * params, ggml_tensor * op) {
        ...
        GGML_ASSERT((src0->ne[1] % NB_COLS) == 0);
        GGML_ASSERT((src0->ne[0] % QK_K) == 0);
        GGML_ASSERT(src1->nb[0] == sizeof(float));
        GGML_ASSERT(src1->nb[1] == sizeof(float) * (size_t) src1->ne[0]);
        ...
        const size_t       row_q8     = ggml_row_size(PARAM_TYPE, src1->ne[0]);
        const int64_t      M          = src1->ne[1];
        const int64_t      M4         = M - (M % 4);
        const ggml_from_float_t from_float = ggml_get_type_traits_cpu(PARAM_TYPE)->from_float;
        GGML_ASSERT(params->wsize >= row_q8 * (size_t) M);
    ...
    void forward_mul_mat_id_q23k(ggml_compute_params * params, ggml_tensor * op) {
        const ggml_tensor * src0 = op->src[0];
        const ggml_tensor * src1 = op->src[1];
        ...
        GGML_ASSERT(ids->ne[0] % src1->ne[1] == 0);
        GGML_ASSERT(src0->ne[0] == src1->ne[0]);
        GGML_ASSERT((src0->ne[1] % NB_COLS) == 0);
        GGML_ASSERT((src0->ne[0] % QK_K) == 0);
        GGML_ASSERT(src1->nb[0] == sizeof(float));
        ...
        GGML_ASSERT(params->wsize >= row_q8 * (size_t) nrows_b);
...
static const ggml::cpu::repack::tensor_traits<block_q2_K, 4, 8, GGML_TYPE_Q8_K> q2_K_8x4_q8_K;
...
static bool ggml_repack_is_q23k_8x4(const void * extra) {
    return extra == (const void *) &q2_K_8x4_q8_K || extra == (const void *) &q3_K_8x4_q8_K;
}

bool ggml_cpu_repack_extra_is_q23k_8x4(const void * extra) {
    return ggml_repack_is_q23k_8x4(extra);
}
...
bool ggml_cpu_repack_extra_is_tiled(const void * extra) {
    if (extra == nullptr) {
        return false;
    }
...
static enum ggml_status ggml_backend_cpu_repack_buffer_init_tensor(ggml_backend_buffer_t buffer, struct ggml_tensor * tensor) {
    ...
    const ggml::cpu::tensor_traits * tr = ggml_repack_get_optimal_repack_type(tensor);
    ...
    if (tensor->view_src != nullptr && ggml_repack_is_q23k_8x4(tr)) {
        GGML_ABORT("%s: views over Q2_K/Q3_K 8x4 packed tensors are not supported", __func__);
    }
    tensor->extra = (void *) const_cast<ggml::cpu::tensor_traits *>(tr);

    // q23k: leave nb[] native (ggml_new_tensor). load_all_data copies
    // n_size = ggml_nbytes from mmap; packed slice padding (and Q3
...
static void ggml_backend_cpu_repack_buffer_set_tensor(ggml_backend_buffer_t buffer, struct ggml_tensor * tensor,
    ...
    if (ggml_repack_is_q23k_8x4(tensor_traits)) {
        // Source is always the native GGUF layout. Packed/padded bytes are
        // dest-only (get_alloc_size). Do not accept ggml_nbytes if nb[] was
        // already rewritten to the tiled stride — that hybrid value is not
        // a legal source size.
        const size_t src_nbytes = ggml_row_size(tensor->type, tensor->ne[0]) *
                                  (size_t) tensor->ne[1] * (size_t) tensor->ne[2] * (size_t) tensor->ne[3];
        if (size != src_nbytes) {
            GGML_ABORT("%s: q23k set size %zu != native source %zu (ggml_nbytes=%zu)",
                       __func__, size, src_nbytes, ggml_nbytes(tensor));
        }
    } else {
...
static size_t ggml_backend_cpu_repack_buffer_type_get_alloc_size(ggml_backend_buffer_type_t buft, const struct ggml_tensor * tensor) {
    ...
    const ggml::cpu::tensor_traits * tr = ggml_repack_get_optimal_repack_type(tensor);
    if (ggml_repack_is_q23k_8x4(tr)) {
        const size_t  tile  = (tensor->type == GGML_TYPE_Q2_K) ? sizeof(block_q2_K_8x4) : sizeof(block_q3_K_8x4);
        const int64_t n_exp = tensor->ne[2] * tensor->ne[3];
        return ggml_repack_q23k_8x4_nbytes(tile, tensor->ne[0], tensor->ne[1], n_exp);
    }
...
class extra_buffer_type : ggml::cpu::extra_buffer_type {
    bool supports_op(ggml_backend_dev_t, const struct ggml_tensor * op) override {
        if (    op->op == GGML_OP_MUL_MAT &&
            ...
            const ggml::cpu::tensor_traits * tr = ggml_repack_get_optimal_repack_type(op->src[0]);
            if (tr) {
            ...
            if (tr) {
                if (ggml_repack_is_q23k_8x4(tr)) {
                    ...
                    if (op->src[2]->type != GGML_TYPE_I32) {
                        return false;
                    }
                    return true;
                }
...
static const block_q2_K_8x4 * ggml_repack_q2_k_8x4_tile(const void * packed, int64_t ne0, int64_t ne1,
                                                        int64_t expert, int64_t row, int64_t block) {
    const size_t stride  = ggml_repack_q23k_8x4_slice_stride(sizeof(block_q2_K_8x4), ne0, ne1);
...
static const block_q3_K_8x4 * ggml_repack_q3_k_8x4_tile(const void * packed, int64_t ne0, int64_t ne1,
                                                        int64_t expert, int64_t row, int64_t block) {
    const size_t stride  = ggml_repack_q23k_8x4_slice_stride(sizeof(block_q3_K_8x4), ne0, ne1);
    const int64_t nblocks = ne0 / QK_K;
    const uint8_t * slice = (const uint8_t *) packed + (size_t) expert * stride;
...
size_t ggml_cpu_repack_q2_k_8x4_nbytes(int64_t ne0, int64_t ne1, int64_t ne2) {
    return ggml_repack_q23k_8x4_nbytes(sizeof(block_q2_K_8x4), ne0, ne1, ne2);
}

size_t ggml_cpu_repack_q3_k_8x4_nbytes(int64_t ne0, int64_t ne1, int64_t ne2) {
    return ggml_repack_q23k_8x4_nbytes(sizeof(block_q3_K_8x4), ne0, ne1, ne2);
}

const char * ggml_cpu_repack_extra_name(const struct ggml_tensor * tensor) {
...
int ggml_cpu_repack_pack_q2_k_8x4(struct ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q2_K_to_q2_K_8x4(t, data, data_size);
}

int ggml_cpu_repack_pack_q3_k_8x4(struct ggml_tensor * t, const void * data, size_t data_size) {
    return repack_q3_K_to_q3_K_8x4(t, data, data_size);
}

void ggml_cpu_repack_dequant_q2_k_8x4(const void * packed, float * dst, int64_t ne0, int64_t ne1, int64_t ne2) {
    const size_t  stride  = ggml_repack_q23k_8x4_slice_stride(sizeof(block_q2_K_8x4), ne0, ne1);
    const int64_t nblocks = ne0 / QK_K;
        ...
        for (int64_t rg = 0; rg < ne1 / 8; rg++) {
            for (int64_t bl = 0; bl < nblocks; bl++) {
                ...
                for (int rowi = 0; rowi < 8; rowi++) {
                    const float d    = GGML_FP16_TO_FP32(tile->d[rowi]);
                    const float dmin = GGML_FP16_TO_FP32(tile->dmin[rowi]);
...
void ggml_cpu_repack_dequant_q3_k_8x4(const void * packed, float * dst, int64_t ne0, int64_t ne1, int64_t ne2) {
    const size_t  stride  = ggml_repack_q23k_8x4_slice_stride(sizeof(block_q3_K_8x4), ne0, ne1);
    const int64_t nblocks = ne0 / QK_K;
    for (int64_t e = 0; e < ne2; e++) {
        const uint8_t * slice = (const uint8_t *) packed + (size_t) e * stride;
        for (int64_t rg = 0; rg < ne1 / 8; rg++) {
            for (int64_t bl = 0; bl < nblocks; bl++) {
                ...
                for (int rowi = 0; rowi < 8; rowi++) {
                    const float d_all = GGML_FP16_TO_FP32(tile->d[rowi]);
                    float * out = dst + ((e * ne1 + rg * 8 + rowi) * ne0 + bl * QK_K);
                    for (int sb = 0; sb < 16; sb++) {
...
uint8_t ggml_cpu_repack_q2_k_8x4_get_sm(const void * packed, int64_t ne0, int64_t ne1,
                                        int64_t expert, int64_t row, int64_t block, int sb) {
    return ggml_repack_q2_k_8x4_tile(packed, ne0, ne1, expert, row, block)->sm[sb][row % 8];
}

uint8_t ggml_cpu_repack_q2_k_8x4_get_q(const void * packed, int64_t ne0, int64_t ne1,
                                       int64_t expert, int64_t row, int64_t block, int sb, int lane) {
    const uint8_t * qs = ggml_repack_q2_k_8x4_tile(packed, ne0, ne1, expert, row, block)->qs[sb][row % 8];
    return (uint8_t) ((qs[lane / 4] >> ((lane % 4) * 2)) & 3);
}

uint8_t ggml_cpu_repack_q3_k_8x4_get_scale_code(const void * packed, int64_t ne0, int64_t ne1,
                                                int64_t expert, int64_t row, int64_t block, int sb) {
    return ggml_repack_q3_k_8x4_tile(packed, ne0, ne1, expert, row, block)->scale_code[sb][row % 8];
}

uint8_t ggml_cpu_repack_q3_k_8x4_get_q(const void * packed, int64_t ne0, int64_t ne1,
                                       int64_t expert, int64_t row, int64_t block, int sb, int lane) {
    const block_q3_K_8x4 * tile = ggml_repack_q3_k_8x4_tile(packed, ne0, ne1, expert, row, block);
