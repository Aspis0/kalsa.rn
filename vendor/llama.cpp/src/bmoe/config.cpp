...
ValidationResult validate(const RunConfig & cfg) {
    ...
    // Empty override strings are a caller typo, not "pin nothing". A missing '=' is a bare regex
    // and is accepted (Session::open treats it as pattern=CPU).
    for (const std::string & ot : cfg.override_tensor) {
        if (ot.empty()) {
            return fail("override_tensor entries must be non-empty (pattern or pattern=BufferType)");
        }
    }
