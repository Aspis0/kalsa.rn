...
enum class DenseWeightsMode {
    ...
    AnonGpu,   // like Anonymous for dense that stayed on the host; skip tensors already in a GPU
               // buffer so a -ngl trunk is not doubled in DRAM. n_gpu_layers==0 behaves as Anonymous.
};

// How routed expert tensors (ffn_*_exps) are pinned to CPU when the trunk may live on GPU.
// Off  = no override (Mechanism A: OpenCL supports_op leaves Q2_K/Q3_K experts on CPU).
// On   = always pin via tensor_buft_overrides (Mechanism B).
// Auto = pin when n_gpu_layers != 0, so a future GPU MUL_MAT_ID cannot steal the expert-ready hook.
enum class MoeCpuExperts {
    Off,
    On,
    Auto,
};

inline bool dense_mode_host_copy(DenseWeightsMode m) {
    return m == DenseWeightsMode::Anonymous || m == DenseWeightsMode::Pinned || m == DenseWeightsMode::AnonGpu;
}

inline const char * dense_weights_flag(DenseWeightsMode m) {
    switch (m) {
        ...
        case DenseWeightsMode::Pinned:
            return "ahwb";
        case DenseWeightsMode::AnonGpu:
            return "anon-gpu";
    }
    return "anon";
}

inline const char * moe_cpu_experts_name(MoeCpuExperts m) {
    switch (m) {
        case MoeCpuExperts::Off:
            return "off";
        case MoeCpuExperts::On:
            return "on";
        case MoeCpuExperts::Auto:
            return "auto";
    }
    return "off";
}
...
struct MoeStreamConfig {
    ...
    // gguf header/metadata, embeddings, attention, norms, lm_head — is handled by one of the
    ...
    //   AnonGpu   same as Anonymous for host-remaining dense (token_embd, any -ot …=CPU); tensors
    //             already in a GPU buffer get no second host copy. Use with --ngl so the trunk has
    //             one DRAM object (the CL buffer). With n_gpu_layers==0 this is Anonymous.
...
struct RunConfig {
    ...
    // GPU trunk / CPU experts. n_gpu_layers==0 is the campaign CPU path (default). Negative means
    // every repeating layer plus the output (llama.cpp convention). Flash Attention stays off unless
    // asked — AUTO is treated as on by llama.cpp and aborts on Adreno A7X.
    int n_gpu_layers = 0;
    bool flash_attn = false;
    // CLI default is Auto only when --moe-stream is set; otherwise Off. See MoeCpuExperts.
    MoeCpuExperts moe_cpu_experts = MoeCpuExperts::Off;
    // Extra tensor buffer overrides, llama.cpp `-ot` form: "pattern=BufferType". A bare regex is
    // treated as pattern=CPU. Repeatable; comma-separated values are split at Session::open.
