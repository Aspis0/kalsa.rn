...
void fill_model_info(const gguf_context * gctx, GgufModelInfo & out) {
    ...
    if (!out.arch.empty()) {
        ...
        // LLM_KV_EXPERT_USED_COUNT / LLM_KV_EMBEDDING_LENGTH expand "%s" to the architecture).
        out.n_embd = meta_int(gctx, out.arch + ".embedding_length", 0);
        const int expert_ff = meta_int(gctx, out.arch + ".expert_feed_forward_length", 0);
        out.n_ff = expert_ff > 0 ? expert_ff : meta_int(gctx, out.arch + ".feed_forward_length", 0);
        out.n_expert = meta_int(gctx, out.arch + ".expert_count", 0);
        out.n_expert_used = meta_int(gctx, out.arch + ".expert_used_count", 0);
        out.n_layer = meta_int(gctx, out.arch + ".block_count", 0);
    }
    out.ok = true;
}
