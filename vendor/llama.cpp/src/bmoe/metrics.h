...
struct RunInfo {
    ...
    std::string dense_weights = "anon"; // dense (non-expert) policy: mmap|warm|anon|ahwb|anon-gpu
    int n_gpu_layers = 0;              // 0 = CPU-only; <0 = all layers + output
    bool flash_attn = false;
    std::string moe_cpu_experts = "off"; // off|on|auto — how routed ffn_*_exps were pinned
