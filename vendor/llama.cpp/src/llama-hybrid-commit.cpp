...
static bool compatible_tensor(const ggml_tensor * dst, const ggml_tensor * src) {
    ...
    return dst->type == src->type && ggml_nbytes(dst) == ggml_nbytes(src);
}
...
static bool copy_tensor(const ggml_tensor * src, ggml_tensor * dst,
    ...
    scratch.resize(size);
    ggml_backend_tensor_get(src, scratch.data(), 0, size);
    ggml_backend_tensor_set(dst, scratch.data(), 0, size);
    if (copied_bytes) {
        *copied_bytes += size;
    }
    return true;
