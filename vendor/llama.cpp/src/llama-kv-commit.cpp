...
struct llama_kv_commit_access {
    ...
    static bool same_host_tensor(const ggml_tensor * dst, const ggml_tensor * src) {
        if (!dst || !src) {
            return dst == src;
        }
    ...
    static bool has_non_host_v(const llama_kv_cache & cache) {
        for (const auto & layer : cache.layers) {
            for (const auto * v : layer.v_stream) {
                if (v && v->buffer && !ggml_backend_buffer_is_host(v->buffer)) {
                    return true;
                }
            }
        }
    ...
    static bool copy_tensor(const ggml_tensor * src, ggml_tensor * dst,
        ...
        scratch.resize(size);
        ggml_backend_tensor_get(src, scratch.data(), src_offset, size);
        if (stats) {
            if (is_v) {
        ...
        ggml_backend_tensor_set(dst, scratch.data(), dst_offset, size);
        if (stats) {
            if (is_v) {
                ++stats->transfers_naive;
                ++stats->v_sets_naive;
    ...
    static bool copy_data(const llama_kv_cache & dst, const llama_kv_cache & src,
        ...
        for (size_t i = 0; i < src.layers.size(); ++i) {
            ...
            const size_t k_size_row = ggml_row_size(src_k->type, src.hparams.n_embd_k_gqa(src_layer.il));
        ...
        if (!src.v_trans) {
            for (size_t i = 0; i < src.layers.size(); ++i) {
                ...
                const size_t v_size_row = ggml_row_size(src_v->type, src.hparams.n_embd_v_gqa(src_layer.il));
                for (const auto & range : ranges) {
                    const size_t range_size = range.second - range.first;
            ...
            for (size_t i = 0; i < src.layers.size(); ++i) {
                ...
                const size_t v_size_el = ggml_type_size(src_v->type);
    ...
    static bool mirror(llama_kv_cache & dst, const llama_kv_cache & src,
                       ...
                       llama_kv_commit_mode mode, ggml_backend_sched_t src_sched,
...
// ggml_backend_tensor_get/set return void, so mid-copy I/O failures are undetectable (known limitation).
